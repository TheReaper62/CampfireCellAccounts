...
async def on_raw_reaction_add(payload):
    ...
    if emoji == "✅" and member!=client.user:
        ...
        users_completion = [f"✅ {member.display_name}" if member.id in completed['completed']['users'] else f"❌ {member.display_name}" for member in member.guild.members if not member.bot]
